...
// fp(7, 3) => 4
```
# <p style="text-align: center;"> 11/2/20 </p>
```C
...
double R_sum (double (*fp) (double), double a, double b, double delta) {
    double sum = 0; 
...
- Modules are also commonly known as **libraries**
  - Libraries are usually sufficiently large and used by many users

## Modules vs. files
In this course, and in most cases, it is considered **good style** to store **modules** in **separate files**. 

## Terminology
The **client** **requires** the **functions** that a module **provides**
- imagine the client is a stranger
  - don't assume there is any context

## Building a program 
When building a program we can combine ("link") **multiple machine code files** together to form a single program. that means we can "build" a program from multiple source code files and/or machine code files
```
main.c module.c module2.c module3.ll
```
- modules do not contain a main function
  - only **one** main function an be defined in a program

## Motivation (benefits of modularization)
...
**Maintainability**: It is much easier to test and debug a single module instead of a large program. If a bug is fond only the module that contains the bug needs to be fixed.

**Abstraction**: To use a module, the client needs to understand **functionality** it provides, but it does not need to understand **how** it is implemented. 

## Declarations
In C, a function or variable mus tbe **declared** before it can b accessed (or referenced)
...
- A definition instructs C to "create" the identifier
    - a definition includes a declaration

A variable declarations starts with extern keywords, followed by the type then the variable name. There is no initialization
```C
extern int g; 
void printg(void) {
    print("%d",g);
...
## Types of scope (revisted)
- Local (block) identifiers
  - only available inside of the function (or bloc)
- global identifiers
  - program scope identifiers, available to any file in the program
  - file scope

## **Module interface**
The module **interface** is the list of functions that the module provides (including the documentation)

The interface is separate from the module **implementation**, which is the code fo the module (i.e., function definitions)
...
- **documentation** (purpose) for each provided function

## Interface (.h) files
For C modules, the interface is placed in a separate file with a `.h` file extentsion

clients can read the documentation in the interface to understand how to use the provided functions

The client can also copy and pase the function definitions from the header file

## #include
A preprocessor directive temporarily modifies a source file just before it is run (but does not save the modifications)

The directive #include cut and pastes the contents of another file directly into your code

## Implementation notes
It is good style to have header files for each module

Referring the reader to interface file is sufficient
```C
...
#include "mymodule.h"`
```
# <p style="text-align: center;"> 13/2/20 </p>
## cs136.h
```C
#include "cs136.h"
```
...
**High cohesion** means that all the interface functions are related and working toward a "common goal". A module with many unrelated interface functions is poorly designed. 
...
- modules should be close to self sufficient (shouldn't have a lot of dependencies)
...
## Information hiding
The two key advantages of information hiding are 
- **security** 
- **flexibility**

**Security**
- we may want to prevent the client from tampering with data used by the module. Even if the tampering is not malicious, we may need to "protect the client from themselves"
...
instead of providing the client with the implementation source code (`.c` file) you can provide a machine code (e.g., a `.ll` file). 
...
There are **disassemblers** which are able to "guess" and reverse engineer binary files to potential source code. 
- however it is usually very hard to comprehend the code these disassemblers produce
...
They are implemented in C using **incomplete declaration**, where a structure is declared without any field. 
...
```C
struct box my_box; st   // Invalid
struct box *box_ptr;      // Valid
```
The reason this works is because the fields for the structure are not defined for the client so a non-malicious user cannot change any of the fields. 
- there are ways for malicious attackers to use the pointer to change values though

If a module only provides an incomplete declaration in the interface the the client cannot create an instance of that structure. 
- Modules with opaque structures must provide a function for creating and destroying structures of that type. 

## Opaque structure example
We will create a small `stopwatch`   module which tracks race times and we do not want clients to be able to tamper with the times
...
// effects: allocates memory (client must call
stopwatch_destroy) struct stopwatch *stopwatch_create(void);
...
// stopwatch_add_time(sw, min, sec) adds min[utes] and // sec[onds] to sw
...
// stopwatch.c [IMPLEMENTATION]
  ...
  // requires: 0 <= min
  //           0 <= sec <= 59
int stopwatch_get_seconds(const struct stopwatch *sw) { assert(sw);
return sw->sec;
}
int stopwatch_get_minutes(const struct stopwatch *sw) { assert(sw);
return sw->min;
}
...
void stopwatch_add_time(struct stopwatch *sw, int min, int sec) { assert(sw);
    assert(sec >= 0);
    assert(min >= 0);
    const int total_time = (min + sw->min) * seconds_per_minute + sec + sw->sec;
...
In the previous `stopwatch` example, we demonstrated two **implementation** with different **data structures**:
...
Formally, an ADT is mathematical model for storing and accessing data through **operations**. As mathematical models they transcend any specific computer language or implementation. 

In practice ADTs are **implemented** as data storage **modules** that only allow access to the data through interface functions (ADT operations). THe underlying data structure and implementation of an ADT are **hidden** from the client (which provides flexibility and security). 

# <p style="text-align: center;"> 25/2/20 </p>
## Arrays
...
char b[3] = "cat";
```
- In this example. a and b are character arrays and are not valid strings. 
...
## Array length
C does not explicitly keep track of the array **length** as part of thiearray data structure 
- you must keep track of the array lenght sperately
...
```C
const int a_len = 6; 
int a[a_len] = {1, 2, 3, 4, 5, 6};  // NOT IN CS136
```
This would appeare to be a "better style"

However the syntax to do this properly is outside the scope of this course. 
...
```
- However in practice this should be avoided, as the sizeof operator only properly reports the size in specific circumstances
...
An array of $k$ elements, each of size *s*, requires exactly $k \cdot s$ bytes. 

In the C memory mode, array elements are adjacent to each other. Each element of an array is placed in memory immediately after the previous element. 
...
trace_ptr(a);      //     a => 0x5000
trace_ptr(&a);     //    &a => 0x5000
trace_ptr(&a[0]);  // &a[0] => 0x500
```
...
> Typically, the length of an array is unknown to the function, and is a separate parameter

There is no method for "enforcing" that the length passed to a function is valid

Functions should **require** that the length is valid, but there is no way for a function to `assert` that requirement. 

As we have seen before, passing an address to a function allows the function to mutate the contents tat hat address. 
```C
...
```
It's good style to use the const keyword to both prevent mutation and communicate that no mutations occurs. `
```C
...
# <p style="text-align: center;"> 27/2/20 </p>
...
> C simply translates `a[j]` to `*(a+j)`, which may be outside the bounds of the array (e.g, `a[100000000]` or `a[-1]`)
...
// output: 0 1 2 3 4
```
- this works because it performs `*(i + a)` which is the same as `*(a + i)` since `a` is an address (some number) and i is an integer. 
...
- the second is slightly preferred as it's more optimized
```C
const int *p;
int const* p; 
int * const p; 
const int const *p; 
```
- consider these and understand the differences
...
## Quicksort 
Quick ort is an example of a "divide & conquer" algorithm. 

First, an element is selected as a "pivot" element. 

The list is then **partitioned** (divided) into two sub-groups: Less than (or equal to) the pivot and those greater than the pivot. 

Finally, each subgroup is then sorted
